...
SMTP diagnosis correction (user correction accepted - verified against live behavior):
- The project uses RESEND custom SMTP (smtp.resend.com), NOT the built-in service. My earlier built-in-quota conclusion is RETRACTED - the live evidence disproves it: (a) a clean-wait single signup got PAST GoTrue-s email rate limiter and executed a real ~3.4s SMTP send attempt that FAILED; (b) an owner-address +alias signup (which Resend test-mode would deliver) ALSO fails - so it is not the test-sender restriction.
- CURRENT failure point: GoTrue passes the request to the configured SMTP (Resend) and the SEND is rejected - 500 unexpected_failure / Error sending confirmation email (error_ids 01a074a7-1a8b-7293-abee-23d45828a1b2, 01a074a8-c767-765f-8d20-8f773ece26fd). Recipient-independent, ~3s SMTP handshake timing.
- Secondary layer: GoTrue-s own email rate limit also fires (429 over_email_send_rate_limit, instant) when the window is exhausted - a dashboard Rate Limits > Email setting, separate from Resend.
- Most probable Resend-side cause (user must confirm in Resend dashboard > Emails/Logs + API Keys): revoked/rotated/disabled SMTP credential (the API key used as the SMTP password) or an account-level sending restriction. Deliveries worked at ~03:00, so credentials/auth were valid then.
- Dashboard checks: Resend > Emails (exact rejection reason per attempt), Resend > API Keys (key exists/enabled), Resend > Domains (verified), Resend > Usage (quota), Supabase > Logs > Auth (error_id entries), Supabase > Authentication > Rate Limits (email sending value).
Resend status page: fully operational, no active incidents (SMTP 99.77 pct uptime) - rules out a provider outage; the rejection is account/credential-level on Resend (dashboard Emails log + API Keys page show the exact reason).
