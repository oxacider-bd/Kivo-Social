...
// ─── GET /api/profiles/:username/followers?cursor ────────────────────────────
...
export const GET = route<Ctx>(async ({ req, user, params }) => {
  ...
  if (profile.isPrivate && !viewer.canViewContent) {
    throw new HttpError(
      "FORBIDDEN",
      "This account is private. Follow them to see their followers.",
    );
  }
  ...
  const rows = await db.follow.findMany({
    where: seek
      ? { AND: [{ followingId: profile.userId }, seek] }
  ...
  for (const row of page.items) {
    if (row.follower.profile) {
      items.push(mapProfileCard(row.follower.profile, authed.id, followingSet, followerSet));
    }
  }
